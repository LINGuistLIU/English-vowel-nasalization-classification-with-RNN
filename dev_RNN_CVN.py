...
traindata_nasal = 'data/all/diss_en/train/CVN/'
devdata_nasal = 'data/all/diss_en/dev/CVN/'

traindata_non = 'data/all/diss_en/train/CVC/'
devdata_non = 'data/all/diss_en/dev/CVC/'

nasal_trainfiles = MFCCfeatures.fileNamelist(traindata_nasal)
nasal_testfiles = MFCCfeatures.fileNamelist(devdata_nasal)
oral_trainfiles = MFCCfeatures.fileNamelist(traindata_non)
oral_testfiles = MFCCfeatures.fileNamelist(devdata_non)
...
def parameterTune(n_hidden, n_iters, learning_rate, plotpath, n_letters = n_letters, n_categories=n_categories, all_categories=all_categories, category_lines=category_lines, seedNum=10):
    overallAccuracyList = []
    ...
    for seed in range(1, seedNum+1):
        ...
        rnn = RNNclassifier.RNNclassifier(n_letters, n_hidden, n_categories).to(device)

        criterion = nn.NLLLoss().to(device) #NLLLoss -- negative log likelihood loss
        ...
        RNNclassifier.plotLosses(all_losses, plotpath+'dev-RNN_Trainingloss_plot_seed'+str(seed)+'.pdf')

        # To plot the performance of the trained model on training data
        # RNNclassifier.plotTrainPerform(rnn, category_lines, all_categories, n_categories = 2, n_confusion = 10000)
...
if __name__ == '__main__':
    n_hidden = int(sys.argv[1]) # hidden size
    n_iters = int(sys.argv[2]) # number of iterations
    ...
    oral_file_pred_dict, nasal_file_pred_dict, \
    nasalBestPredDict, oralBestPredDict \
    = parameterTune(n_hidden, n_iters, learning_rate, plotpath)

    #plot the result
    cxt = 'CVN'
    datatype = 'CVC-vs-' + cxt
    plotname= plotpath + 'devCM_RNN_'+ datatype + '.pdf' # name of the confusion matrix plot
