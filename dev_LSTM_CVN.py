...
def parameterTune(n_hidden, n_iters, learning_rate, plotpath, n_letters = n_letters, n_categories=n_categories, all_categories=all_categories, category_lines=category_lines, seedNum=10):
    ...
    for seed in range(1, seedNum+1):
        ...
        rnn = LSTMclassifier.LSTMclassifier(n_letters, n_hidden, n_categories)
        ...
        # To train the model
        all_losses = LSTMclassifier.trackTraining(rnn, criterion, optimizer, all_categories, category_lines, n_iters, print_every = 1000, plot_every = 100)
        print('all_losses\t', all_losses)
        LSTMclassifier.plotLosses(all_losses, plotpath+'dev-LSTM_Trainingloss_plot_seed'+str(seed)+'.pdf')
        ...
        for soundfile in test_cfdict['NASAL']:
            test_guess_nasal += 1
            actualNasals += 1
            pred = LSTMclassifier.predict(rnn, soundfile, all_categories, n_predictions=1)
...
if __name__ == '__main__':
    ...
    cxt = 'CVN'
    datatype = 'CVC-vs-'+cxt
    plotname= plotpath+'devCM_LSTM_'+ datatype + '.pdf' # name of the confusion matrix plot
